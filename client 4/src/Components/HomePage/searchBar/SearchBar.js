...
function SearchBar() {
  ...
  const isSearchAble =
    city && people && calendar[0].startDate && calendar[0].endDate;
  ...
  const searchHanlder = () => {
    dispatch(
      updateDataSearch({
        city,
        timeRange: [
          calendar[0].startDate.toDateString(),
          calendar[0].endDate.toDateString(),
        ],
        people,
      })
    );
  ...
  const startDate = calendar[0].startDate
    ? calendar[0].startDate?.toLocaleDateString("en-GB")
    : undefined;
  const endDate = calendar[0].endDate
    ? calendar[0].endDate?.toLocaleDateString("en-GB")
    : undefined;
